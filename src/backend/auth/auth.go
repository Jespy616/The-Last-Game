...
func Register(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		var ve gin.H = gin.H{"errors": []string{}}

		// Parse each error field into a user-friendly string
		for _, fieldErr := range err.(validator.ValidationErrors) {
			switch fieldErr.Field() {
			case "Email":
				ve["errors"] = append(ve["errors"].([]string), "Email is required and must be valid.")
			case "Username":
				ve["errors"] = append(ve["errors"].([]string), "Username is required.")
			case "Password":
				ve["errors"] = append(ve["errors"].([]string), "Password is required.")
			default:
				ve["errors"] = append(ve["errors"].([]string), fieldErr.Error())
			}
		}
